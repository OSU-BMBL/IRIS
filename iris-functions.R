#---------------------------------------------------------------------
# Title:         IRIS - Core Functions
# Author:        Brandon Monier
# Created:       2018-01-26 11:31:20 CDT
# Last Modified: 2018-05-16 15:36:07 CDT
#---------------------------------------------------------------------
...
#---------------------------------------------------------------------
#
#   IRIS core function organization
#
...
#       05 PLOT....... Miscellaneous plotting functions
#       06 XLSX....... Excel metadata return functions
#
#---------------------------------------------------------------------
#
#   IRIS core function layout
#
#       IRIS
#       |-  HOUSE
#           |- CRAN install
...
#               |- `getSizeFact()`
#           |- Get normalized counts from different objects
#               |- `getNormCounts()`
...
# CTS overview (similar to a tribble)
trubble <- function(cts) {
    ...
    if (ncol(tmp) > 10) {
        output <- paste(
            "Your pre-processed data contains", nr, "IDs and", nc, 
            "samples. Showing the first 10 samples:\n"
        )
    } else {
        output <- paste(
            "Your pre-processed data contains", nr, "IDs and", 
            nc, "samples.\n"
        )
    }
...
###################################################################
###################################################################
### SECTION 03 - LIMMA RETURN FIT FUNCTIONS (EDGER-RF)
###################################################################
###################################################################
...
###################################################################
###################################################################
### SECTION 06 - EXCEL RETURN FUNCTIONS (XLSX)
###################################################################
###################################################################
...
cts_split <- function(cts) {
    ...
    wd <- paste0(
        "tmp-", 
        paste(sample(0:9, 6, replace = TRUE), collapse = "")
    )
    if (file.exists("tmp")) {
        setwd("tmp")
...
pdf_sorter <- function(pdf_list, samples) {
    ...
    l4 <- lapply(seq(from = 2, to = n), function(i) {
        l3[grep(paste0(
            "^geo_pdfile_[0-9]{3}_", str_pad(i, 3, pad = 0)),names(l3))]
    })
...
# XLSX - Character data file merger
char_sorter <- function(pdf_list, samples) {
    ...
    l4 <- lapply(seq(from = 1, to = n), function(i) {
        l3[grep(paste0("^geo_character_[0-9]{3}_", 
            str_pad(i, 3, pad = 0)),names(l3))]
    })
